/**
  ******************************************************************************
  * @file    stm32l1xx_ll_lcd.h
  * @author  GORAGOD
  * @brief   Header file of LCD Controller LL module.
  ******************************************************************************/

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __STM32L1xx_LL_LCD_H__
#define __STM32L1xx_LL_LCD_H__
...
typedef enum
{
  LL_LCD_STATE_RESET             = 0x00,    /*!< Peripheral is not yet Initialized */
  LL_LCD_STATE_READY             = 0x01,    /*!< Peripheral Initialized and ready for use */
  LL_LCD_STATE_BUSY              = 0x02,    /*!< an internal process is ongoing */
  LL_LCD_STATE_TIMEOUT           = 0x03,    /*!< Timeout state */
  LL_LCD_STATE_ERROR             = 0x04     /*!< Error */
}LL_LCD_StateTypeDef;
...
#define LL_LCD_ERROR_NONE      (0x00U)    /*!< No error */
#define LL_LCD_ERROR_FCRSF     (0x01U)    /*!< Synchro flag timeout error */
#define LL_LCD_ERROR_UDR       (0x02U)    /*!< Update display request flag timeout error */
#define LL_LCD_ERROR_UDD       (0x04U)    /*!< Update display done flag timeout error */
#define LL_LCD_ERROR_ENS       (0x08U)    /*!< LCD enabled status flag timeout error */
#define LL_LCD_ERROR_RDY       (0x10U)    /*!< LCD Booster ready timeout error */
...
/** @brief  macros to enables or disables the LCD
  ...
  * @retval None
  */     
#define __LL_LCD_ENABLE()              (SET_BIT(LCD->CR, LCD_CR_LCDEN))
#define __LL_LCD_DISABLE()             (CLEAR_BIT(LCD->CR, LCD_CR_LCDEN))
...
/** @brief  Macros to enable or disable the low resistance divider. Displays with high 
  ...
  * @retval None
  */
#define __LL_LCD_HIGHDRIVER_ENABLE()                                \
                              do{                                              \
                                  SET_BIT(LCD->FCR, LCD_FCR_HD); \
                                  LCD_WaitForSynchro();              \
                                 }while(0)

#define __LL_LCD_HIGHDRIVER_DISABLE()                               \
                              do{                                              \
                                  CLEAR_BIT(LCD->FCR, LCD_FCR_HD); \
                                  LCD_WaitForSynchro();              \
                                 }while(0)
...
/**
  ...
  * @retval None
  */
#define __LL_LCD_PULSEONDURATION_CONFIG(__DURATION__)                \
        do{                                                                       \
            MODIFY_REG(LCD->FCR, LCD_FCR_PON, (__DURATION__)); \
            LCD_WaitForSynchro();                                       \
           }while(0)
...
/**
  ...
  * @retval None
  */
#define __LL_LCD_DEADTIME_CONFIG(__DEADTIME__)                        \
        do{                                                                        \
            MODIFY_REG(LCD->FCR, LCD_FCR_DEAD, (__DEADTIME__)); \
            LCD_WaitForSynchro();                                        \
           }while(0)
...
/**
  ...
  * @retval None
  */
#define __LL_LCD_CONTRAST_CONFIG(__CONTRAST__)                       \
        do{                                                                       \
...
/**
  ...
  * @retval None
  */
#define __LL_LCD_BLINK_CONFIG(__BLINKMODE__, __BLINKFREQUENCY__)  \
        do{                                                                    \
...
/** @brief  Enables or disables the specified LCD interrupt.
  ...
  * @retval None
  */
#define __LL_LCD_ENABLE_IT(__INTERRUPT__)                                \
                         do{                                                          \
                             SET_BIT(LCD->FCR, (__INTERRUPT__));   \
                             LCD_WaitForSynchro();                          \
                            }while(0)         
#define __LL_LCD_DISABLE_IT(__INTERRUPT__)                               \
                         do{                                                          \
                             CLEAR_BIT(LCD->FCR, (__INTERRUPT__)); \
                             LCD_WaitForSynchro();                          \
                            }while(0)
...
/** @brief  Checks whether the specified LCD interrupt is enabled or not.
  ...
  */
#define __LL_LCD_GET_IT_SOURCE(__IT__) ((LCD->FCR) & (__IT__))
...
/** @brief  Checks whether the specified LCD flag is set or not.
  ...
  */
#define __LL_LCD_GET_FLAG(__FLAG__)    ((LCD->SR & (__FLAG__)) == (__FLAG__))   
...
/** @brief  Clears the specified LCD pending flag.
  ...
  * @retval None
  */
#define __LL_LCD_CLEAR_FLAG(__FLAG__)  (LCD->CLR = (__FLAG__))
...
LL_LCD_StateTypeDef     		LL_LCD_Init(LCD_InitTypeDef*);
void        								LL_LCD_MspInit(void);
void        								LL_LCD_MspDeInit(void);
...
LL_LCD_StateTypeDef     LL_LCD_Clear(void);
